...
#define MemBuf_MAXDELTA    1048576
...
// MemBuf::operator=
//-------------------------------------------------------------------
MemBuf& MemBuf::operator=(const MemBuf& buf)
{
  if( !m_bOwner )
    reset();
  uint32 uiNb = buf.len();
...
// MemBuf::operator==
//-------------------------------------------------------------------
bool MemBuf::operator==(const MemBuf &mb) const
{
  if ( m_uiLen == mb.len() )
...
//-------------------------------------------------------------------
// MemBuf::realloc_with_margin
//-------------------------------------------------------------------
void MemBuf::realloc_with_margin(uint32 uiNewSize)
{
  // Min size
  if( uiNewSize < MEMBUF_MINSIZE )
    uiNewSize = MEMBUF_MINSIZE;

  // Not enough space : The buffer grow with MemBuf_MAXDELTA bytes max
  uint32 uiDelta = uiNewSize / 2;
  if (uiDelta > MemBuf_MAXDELTA)
    uiDelta = MemBuf_MAXDELTA;

  uint32 uiNewLenBuf = uiNewSize + uiDelta;
  realloc(uiNewLenBuf);
}
...
// MemBuf::set_len
//-------------------------------------------------------------------
void MemBuf::set_len(uint32 uiNb)
{
  if( uiNb > m_uiLenBuf )
...
// MemBuf::put_bloc
//-------------------------------------------------------------------
void MemBuf::put_bloc(const void* p, uint32 uiNb)
{
//##  ASSERT( (uint32)m_uiLen + (uint32)uiNb <= (uint32)MAX_ALLOC_NEW)
  uint32 uiTotalLen = m_uiLen + uiNb;
  if( uiTotalLen > m_uiLenBuf )
  {
    // Not enough space : re-alloc with margin
    realloc_with_margin(uiTotalLen);
  }
...
// MemBuf::insert_bloc
//-------------------------------------------------------------------
void MemBuf::insert_bloc(const void* p, uint32 uiNb, uint32 uiInsPos)
{
...
// MemBuf::get_bloc
//-------------------------------------------------------------------
int MemBuf::get_bloc(void* p, uint32 uiNb)
{
...
// MemBuf::move_bloc
//-------------------------------------------------------------------
void MemBuf::move_bloc(uint32 uiDst, uint32 uiSrc, uint32 uiSize)
{
...
// MemBuf::get_crc
//-------------------------------------------------------------------
uint32 MemBuf::get_crc() const
{
  return crc( (byte *)m_pBuf, m_uiLen );
}
...
//-------------------------------------------------------------------
/*
int MemBuf::LoadFile(const char *pszFile)
{
  int rc = 0;
  CFileEx f (pszFile);
...
// MemBuf::SaveToFile
//---------------------------------------------------------------------------
int MemBuf::SaveToFile(const CFileName& fnFile, const char *pszMode,
  uint32 uiShareMode) const
{
  int iRc = 0;
  CFileEx f(fnFile);
...
// MemBuf::attach
//---------------------------------------------------------------------------
void MemBuf::attach(char* pBuf,uint32 uiLen,bool bOwner)
{
  reset();
  m_uiLen = uiLen;
  m_uiLenBuf = uiLen;
...
//---------------------------------------------------------------------------
// MemBuf::GiveOwnership
//---------------------------------------------------------------------------
int MemBuf::give_ownership(MemBuf* pToHaveOwnership)
{
  if( !m_bOwner )
  {
//##    ASSERT_FAILURE;
...
MemBuf& MemBuf::operator<<(uint64 ui64)
{
  #ifdef __MOTOROLA_ENDIAN__
    invert_int64(&ui64);
  #endif
...
MemBuf& MemBuf::operator<<(const char* psz)
{
  put_bloc(psz, strlen(psz)+1);
  return *this;
}

MemBuf& MemBuf::operator<<(const std::string& str)
{
  put_bloc(str.c_str(), str.size()+1);
  return *this;
}

MemBuf& MemBuf::operator>>(std::string& str)
{
  if ( m_pBuf != NULL )
  {
    uint32 uiNb = strlen(cur_pointer());
    str.reserve(uiNb);
    str.append(cur_pointer(), uiNb);
  }
  return *this;
}

MemBuf& MemBuf::operator<<(const MemBuf& membuf)
{
  put_bloc( membuf.buf(), membuf.len() );
  return *this;
}
