...
public:
  ...
  /// Create directory
  void mkdir(mode_t mode = 0, uid_t uid = (uid_t)-1, gid_t gid = (uid_t)-1) const throw(Exception);

  /// Return true the file name is a existing link
  bool link_exist() const throw(Exception);

  /// Make a symbolic link
  void make_sym_link(const String &strTarget, uid_t uid = (uid_t)-1, gid_t gid = (uid_t)-1) const throw(Exception);

  /// Remove file
  void remove() throw(Exception);
  
  /// Remove directory
  void rmdir(bool bRecursive=false, bool bContentOnly=false) throw(Exception);
  ...
  void dir_copy(const String &strDest, bool bCreateDir=false, mode_t modeDir = 0, uid_t uid = (uid_t)-1, gid_t gid = (uid_t)-1) throw(Exception);

  /// Return size in octets
  uint32 size() const throw(Exception);

  /// Return size in octets: 64bits version for files bigger than 2GB
  uint64 size64() const throw(Exception);
  
  /// Rename the file
  void rename(const String &strNewName) throw(Exception);
  
  /// Copy the file to the specified destination with given 
  void copy(const String &strDest, bool bKeepMetaData=false) throw(Exception);
  
  /// Move file
  ///
  void move(const String &strDest) throw(Exception);

  // Returns last modification date & time
  void mod_time(Time *pTm, bool bLocalTime=false) const throw(Exception);

  // Sets last modification date & time
  void set_mod_time(const Time& tm) const throw(Exception);

  /// Change file rights
  void chmod(mode_t mode) throw(Exception);

  /// Recursively change rights for directory and files
  void recursive_chmod(mode_t modeFile, mode_t modeDir, bool bCurrentLevel=true) throw(Exception);

  /// Recursively change directory rights
  void recursive_chmod_dir(mode_t mode) throw(Exception);

  /// Recursively change file rights
  void recursive_chmod_file(mode_t mode) throw(Exception);

  /// Change file owner
  void chown(uid_t uid, gid_t gid = (uid_t)-1) throw(Exception);

  /// Recursively change file owner
  void recursive_chown(uid_t uid, gid_t gid = (uid_t)-1) throw(Exception);

  // Returns filesystem type
  FSType file_system_type() const throw(Exception);

  // Returns filesystem type
  fsid_t file_system_id() const throw(Exception);
};
...
class YAT_DECL File: public FileName
{
  ...
  void load(MemBuf *pMemBuf) throw(Exception);

  // Load file to the given String
  void load(String *pString) throw(Exception);

  // Save file from the given String
  void save(const String &strContent) throw(Exception);
};
...
public:
  ...
  FileEnum(const String &strPath, EEnumMode eMode=ENUM_FILE) throw(BadPathException);
  ~FileEnum();
  ...
  void init(const String &strPath, EEnumMode eMode=ENUM_FILE) throw(BadPathException);

  /// Find next file
  /// @param pointer to a string to put the file name
  /// @return true if a file has been found, false if not
  bool find() throw(BadPathException, FileNotFoundException, Exception);
...
public:
	...
	bool set_section(const String &strSection, bool bThrowException=true) throw( Exception );
...
public:
	...
	/// c-tor
	DirectoryWatcher(const String &strDirectoryPath, WatchMode eMode = ENUM_FIRST) throw( Exception);
	...
	void get_changes(FileNamePtrVector *vecNewFilesPtr, FileNamePtrVector *vecChangedFileNamePtr=NULL,
                    FileNamePtrVector *vecRemovedFileNamePtr=NULL) throw( Exception);
